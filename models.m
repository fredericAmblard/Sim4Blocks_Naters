...
syms cp_w mw_dot
syms T_room T_room_dot T_floor T_floor_dot T_ext T_return T_return_dot T_mid1 T_mid1_dot T_mid2 T_mid2_dot T_supply 
syms K_fr K_ra K_wf 
...
eq_room = C_room * T_room_dot == K_fr * (T_floor - T_room) - K_ra * (T_room - T_ext) + P_gains + P_sol + zero * (T_return + T_supply + T_mid1 + T_mid2);
...
eq_floor = C_floor * T_floor_dot == - K_fr * (T_floor - T_room) + K_wf/2 * (T_return - T_floor) +  K_wf/2 * (T_mid1 - T_floor) + zero * (P_gains + P_sol + T_ext + T_supply + T_mid2);

eq_mid1 = C_return/3 * T_mid1_dot == - K_wf/3 * (T_mid1 - T_floor) + cp_w * mw_dot * (T_supply - T_mid1) + zero * (P_gains + P_sol + T_ext + T_room + T_return + T_mid2);

eq_mid2 = C_return/3 * T_mid2_dot == - K_wf/3 * (T_mid2 - T_floor) + cp_w * mw_dot * (T_mid1 - T_mid2) + zero * (P_gains + P_sol + T_ext + T_room + T_return + T_supply);
...
eq_return = C_return/3 * T_return_dot == - K_wf/3 * (T_return - T_floor) + cp_w * mw_dot * (T_mid2 - T_return) + zero * (P_gains + P_sol + T_ext + T_room + T_supply + T_mid1);
...
equations = solve([eq_room; eq_floor; eq_mid1; eq_mid2; eq_return], states_derivatives);
